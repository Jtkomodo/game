...
public class AABB {
	...
	private boolean DebugColisions=Start.DEBUGCOLISIONS,colide=false,COLIDECHECK=false;
   ...
   public Vector2f findVector(Vector2f position, Vector2f movement,Vector2f direction,AABB box) {
	...
	if(this.colide) {
		   ...
		   float dot=VectorMath.dotProduct(norm, new Vector2f(0,1));
		...
		if(Math.abs(direction.x)!=Math.abs(direction.y)){
		 ...
		 if(closest.x==box.lc.x) {
		   ...
		   edgep.add(edgep.mul(0.00001f,new Vector2f(0,0)));
